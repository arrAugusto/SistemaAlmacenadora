...
$(document).on("click", ".btnAct", function() {
    Swal.fire({
        title: '¿Deseas activar la tarifa?',
        text: "Favor termina de configurar otros gastos para actualizar la nueva tarifa",
        type: 'info',
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
        cancelButtonColor: '#d33',
        confirmButtonText: 'Activar la tarifa'
    }).then((result) => {
        if (result.value) {
            window.location = "activarTarifas";
        }
    })
});
            ...
            $.ajax({
                ...
                success: function(respuesta) {
                    if (respuesta["estadoTarifa"] == 1) {
                        document.getElementById('btnActivaImpreme').innerHTML += '<button type="button" class="btn btn-success btn-sm btnActivacion" numeroParaActivar=0>Activada</button>';
                    } else {
                        document.getElementById('btnActivaImpreme').innerHTML += '<button type="button" class="btn btn-danger btn-sm btnActivacion" numeroParaActivar=1>Descativada</i></button>';
                    }
                                    ...
                                    if (respuesta == "SD") {
                                        ...
                                        for (var i = 0; i < respuesta.length; i++) {
                                            if (respuesta[i]['claveAlmacenaje'] == null) {
                                                var resBotonAlmacenaje = '<button type="button" class="btn btn-danger btnServicios" BotonAlmacenaje=BotonAlmacenaje' + i + 'data-toggle="modal" data-target="#UpdateServiciosAlmacenajes"><i class="fa fa-close"></i></button>';
                                            } else {
                                            ...
                                            if (respuesta[i]['claveSeguro'] == null) {
                                                var resBotonSeguro = '<button type="button" class="btn btn-danger btnServiciosSeguro" BotonSeguro=BotonSeguro' + i + '><i class="fa fa-check"></i></button>';
                                            } else {
                                            ...
                                            if (respuesta[i]['claveManejo'] == null) {
                                                var resBotonManejo = '<button type="button" class="btn btn-danger btnServiciosManejo" BotonManejo=BotonManejo' + i + '><i class="fa fa-check"></i></button>';
                                            } else {
                                            ...
                                            if (respuesta[i]['claveGtosAdmin'] == null) {
                                                var resBotonGtosAdmin = '<button type="button" class="btn btn-danger btnGastosAdmin" BotonGstsAdmin=BotonGstsAdmin' + i + ' ><i class="fa fa-check"></i></button>';
                                            } else {
                                            ...
                                            if (respuesta[i]['claveOtrosGtos'] == null) {
                                                var resBotonOtrosGastos = '<button type="button" class="btn btn-danger btnOtrosGastos" BotonOtrosGts=BotonOtrosGts' + i + ' data-toggle="modal" data-target="#UpdateOtrosGastos"><i class="fa fa-close"></i></button>';
                                            } else {
...
$(document).on("click", ".btnActivacion", function() {
    ...
    var idNuevaTarifa = document.getElementById("lblClienteId").value;
    var datos = new FormData();
...
$(document).on("click", ".bntEditarServicio", function() {
    ...
    if (servicio !== "servicio5") {
        if (idParaActivar == 0) {
            ...
            Swal.fire('Modificacion de servicio', 'Para modificar con exito presione el boton  "SIN GUARDAR"', 'success')
            ...
            Swal.fire('Modificado', 'Modifico con exito el servicio, para rectificar presione el boton amarillo', 'success')
        ...
        if (idParaActivar == 0) {
            ...
            $(this).html('Sin guardar');
            $(this).attr('numeroParaActivar', 1);
            document.getElementById("servicio5").readOnly = false;
            Swal.fire('Modificacion de servicio', 'Para modificar con exito presione el boton  "SIN GUARDAR"', 'success')
        } else if (idParaActivar == 1) {
            var datos = new FormData();
            datos.append("numeroServicio", numeroServicio);
            ...
            $.ajax({
                ...
                success: function(respuesta) {
                    ...
                    $(this).html('Modificado');
                    $(this).attr('numeroParaActivar', 3);
                    document.getElementById("servicio5").readOnly = true;
                    Swal.fire('Modificado', 'Modifico con exito el servicio, para rectificar presione el boton amarillo', 'success')
                }
            })
        } else if (idParaActivar == 3) {
