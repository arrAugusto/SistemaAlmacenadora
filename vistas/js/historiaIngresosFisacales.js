...
$(document).ready(function () {
    if ($("#tablasPilotosUnidades").length >= 1) {
        $.ajax({
            "bServerSide": true,
            url: "ajax/datosGeneralesPolizas.ajax.php",
            success: function (respuesta) {
                console.log(respuesta);
            }
...
$(document).ready(function () {
    if ($("#tablasPilotosUnidades").length >= 1) {
        $('#tablasPilotosUnidades').DataTable({
            "bProcessing": true,
            "sAjaxSource": "ajax/datosGeneralesPolizas.ajax.php",
            "deferRender": true,
            "language": {
                "sProcessing": "Procesando...",
                "sLengthMenu": "Mostrar _MENU_ registros",
        ...
        Swal.fire({
            ...
            if (result.value) {
                ...
                if (resp[0] != "SD") {
                    ...
                    Swal.fire({
                        title: 'Póliza no existe',
                        text: "La póliza no existe, favor revise el dato ingresado!",
                        type: 'error',
                        confirmButtonColor: '#3085d6',
                        confirmButtonText: 'Ok!'
                    }).then(async function (result) {
